# !/usr/bin/python
# -*- coding: UTF-8 -*-
import re, sys, time, uuid, requests, json, psycopg2
...
def write_to_pg(flag):
    conn = psycopg2.connect(database="blog", user="blog", password="blog123456", host="127.0.0.1", port="5432")
    cursor=conn.cursor()
    cursor.execute("update blog_userprofile set info='{}' where user_id=2".format(flag))
    conn.commit()
    conn.close()
